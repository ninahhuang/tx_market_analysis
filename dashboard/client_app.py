...
with explore_tab:
    ...
    st.markdown(
        compact_html(
            """
            <h3 class="comparison-subheading">
                Comparison Snapshot
            </h3>

            <div class="section-subtitle">
                The most important differences across the
                selected markets.
            </div>
            """
        ),
    ...
    snapshot_display = comparison_data[
        [
            "City",
            "Preference_Match_Score",
            "Long_Term_Fundamentals_Score",
    ...
    snapshot_display["Main Consideration"] = (
    ...
    for column in [
        ...
        snapshot_display[column] = (
            snapshot_display[column]
            .map(lambda value: f"{value:.2f}")
        )

    snapshot_display["Confidence"] = (
        snapshot_display["Confidence"]
        .map(lambda value: f"{value:.2f}%")
    )

    snapshot_table_html = snapshot_display.to_html(
        index=False,
    ...
    with st.expander("View all market measurements"):

        detailed_display = comparison_data[
            [
                "City",
                "Population_CAGR_Pct",
                "Five_Yr_CAGR_Pct",
        ...
        detailed_display = detailed_display.rename(
            columns={
                "Population_CAGR_Pct": "Population Growth",
                "Five_Yr_CAGR_Pct": "5-Year Value Growth",
                "Months_of_Supply": "Months of Supply",
                "Latest_Permits_Per_1000": "Permits per 1,000",
                "Inventory_YoY_Pct": "Inventory Growth",
                "Homes_Sold_YoY_Pct": "Home Sales Growth",
                "Sale_to_List_Pct": "Sale-to-List Ratio",
                "Current_Market_Regime": "Market Condition",
            }
        )
